...
def ChannelAverage(feat_matrix):
    ...
    for a in range(feat_matrix.shape[0]):
        total_feat_map = np.zeros((feat_matrix.shape[2], feat_matrix.shape[3]))
        for featmap_channel in range(feat_matrix.shape[1]): 
            feat_map_layer = feat_matrix[a, featmap_channel, :, :] 
            total_feat_map = np.add(total_feat_map, feat_map_layer)
        average_feat_map = total_feat_map / feat_matrix.shape[1]
